...
def simple_models(test_data):
    ...
    output = tf.keras.layers.Dense(1, activation="relu")(tf.keras.layers.Flatten()(dense))
    model = tf.keras.Model(inputs, output)
    model.set_weights([np.random.random(w.shape) * 0.1 for w in model.weights])
    model_plus_one = tf.keras.Model(inputs, output)
    model_plus_one.set_weights([np.random.random(w.shape) * 0.1 for w in model_plus_one.weights])
...
def test_predict_one_time_vs_predict_batch(test_data, simple_models):
    ...
    batch_predictions = predict_batch(solar, sunspots, prediction_times, [model],
                                      [model_plus_one], norm_df)
    # only test 5 rows, otherwise too slow
    batch_predictions = batch_predictions.iloc[:5]
    one_time_predictions = pd.DataFrame(index=batch_predictions.index, columns=batch_predictions.columns)
    one_time_predictions[["timedelta", "period"]] = batch_predictions[["timedelta", "period"]]
    one_time_predictions[["prediction_t", "prediction_t_plus_1"]] = one_time_predictions[["prediction_t", "prediction_t_plus_1"]].astype(float)
    for i in one_time_predictions.index:
        row = one_time_predictions.loc[i]
        t = row["timedelta"]
        period = row["period"]
        time_filter = (solar["timedelta"] < t) & (solar["timedelta"] >= (t - dt.timedelta(days=7)))
        period_filter = solar["period"] == period
        solar_wind_7d = solar.loc[time_filter & period_filter]
        latest_sunspot_number = sunspots.loc[(sunspots["timedelta"] <= t) & (sunspots["period"] == period), "smoothed_ssn"].values[-1]
        pred_t, pred_t_plus_1 = predict_one_time(solar_wind_7d, latest_sunspot_number,
                                                 [model], [model_plus_one], norm_df)
        one_time_predictions.loc[i, ["prediction_t", "prediction_t_plus_1"]] = [pred_t, pred_t_plus_1]
    np.testing.assert_allclose(batch_predictions[["prediction_t", "prediction_t_plus_1"]].values,
                               one_time_predictions[["prediction_t", "prediction_t_plus_1"]].values, atol=1e-3, rtol=1e-3)
